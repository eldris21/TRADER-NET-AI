...
Traducción a operativa (decisiones del usuario, 2026-08-13):
  - Timeframes de estructura: M1 y M5, independientes (config.TFS_ESTRUCTURA).
...
#  - _zonas_vivas:    zonas vigentes por símbolo y timeframe, usadas por
#                     bot_engine para cancelar órdenes límite huérfanas.
_zonas_emitidas: set[str] = set()
_zonas_vivas: dict[str, dict[str, Optional[dict]]] = {}
...
def _construir_senal(symbol: str, zona: dict) -> Optional[Senal]:
    ...
    return Senal(
        symbol=symbol,
        estrategia=f"IGZ-GoldenZone-{tf}",
...
def evaluar_todos_los_simbolos(simbolos: list[str]) -> list[Senal]:
    """
    Recalcula la estructura completa por símbolo EN CADA TIMEFRAME de
    config.TFS_ESTRUCTURA (resistente a reinicios) y devuelve una señal
    por cada zona NUEVA. Cada TF es independiente: M1 y M5 pueden tener
    su propia zona y su propia orden al mismo tiempo.
    Deja el estado de zonas vivas disponible vía zonas_vivas().
    """
    senales: list[Senal] = []
    for symbol in simbolos:
        _zonas_vivas[symbol] = {}
        for tf in config.TFS_ESTRUCTURA:
            cantidad = config.VELAS_POR_TF.get(tf, 600)
            df = data_engine.obtener_velas(symbol, tf, cantidad)
            if df.empty or len(df) < config.ATR_PERIOD * 3:
                logger.warning("%s %s: velas insuficientes para evaluar estructura.", symbol, tf)
                _zonas_vivas[symbol][tf] = None
                continue

            zona = _procesar_simbolo(symbol, df)
            if zona is not None:
                zona["tf"] = tf
            _zonas_vivas[symbol][tf] = zona
            ...
            logger.info("%s: %d velas %s analizadas — zona %s viva [%.5f - %.5f]%s.",
                        symbol, len(df), tf, zona["direccion"],
                        zona["bot"], zona["top"], " (ya mitigada)" if zona.get("tapped") else "")

            zona_id = f"{symbol}|{tf}|{zona['direccion']}|{zona['creada_en']}"
            ...
            logger.info(
                "%s %s: nueva Golden Zone %s [%.5f - %.5f], entrada límite %.5f, ancla %.5f",
                symbol, tf, zona["direccion"], zona["bot"], zona["top"],
...
def zonas_vivas(symbol: str) -> list[dict]:
    """Zonas vigentes del símbolo (todas los TFs) tras la última evaluación."""
    return [z for z in _zonas_vivas.get(symbol, {}).values() if z is not None]
